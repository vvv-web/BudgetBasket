...
class BudgetItemService:
    ...
    def _normalize_patch(item: dict, patch: dict, economist: bool) -> dict:
        if economist:
            ...
            if status == "accepted" and allowed.get("sum_fact") is None and item.get("sum_fact") is None:
                allowed["sum_fact"] = item["sum_plan"]
            if status == "accepted_adjusted" and allowed.get("sum_fact", item.get("sum_fact")) is None:
                raise HTTPException(status_code=400, detail="Для принятия с корректировкой нужна утвержденная сумма")
            if status == "rejected" and allowed.get("sum_fact", item.get("sum_fact")) not in (None, 0):
                raise HTTPException(status_code=400, detail="Для отказа утвержденная сумма должна быть 0 или пустой")
            return allowed
        return {key: patch[key] for key in ("dds_id", "invest_id", "category_id", "sum_plan") if key in patch}
    ...
    def patch_item(self, user: dict, item_id: str, patch: dict) -> dict:
        ...
        if request["status"] == "fixed":
            raise HTTPException(status_code=400, detail="Зафиксированную заявку нельзя редактировать")
