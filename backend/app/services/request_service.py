...
class RequestService:
    ...
    def summary(self, request_id: str) -> dict:
        ...
        accepted = [item for item in items if item["status"] in {"accepted", "accepted_adjusted"}]
        rejected = [item for item in items if item["status"] == "rejected"]
        in_review = [item for item in items if item["status"] == "in_review"]
        return {
    ...
    def list_requests(self, user: dict, budget_year: int | None = None, status: str | None = None, unit_id: str | None = None) -> list[dict]:
        ...
        for request in requests:
            if visible is not None and request["id"] not in visible:
                continue
            if budget_year and request.get("budget_year", 2026) != budget_year:
                continue
    ...
    def patch_request(self, user: dict, request_id: str, patch: dict) -> dict:
        request = get_required(self.repo, "requests", request_id)
        if user["role"] == "admin":
            return self.public_request(self.repo.update("requests", request_id, {key: value for key, value in patch.items() if key in {"economist_id", "unit_id", "sum", "status"}}), self.summary(request_id))
    ...
    def submit(self, user: dict, request_id: str) -> dict:
        ...
        return self.public_request(self.repo.update("requests", request_id, {"status": "submitted"}), self.summary(request_id))

    def start_review(self, user: dict, request_id: str) -> dict:
        request = get_required(self.repo, "requests", request_id)
        self.permissions.require_economist_review_request(user, request)
        if request["status"] not in {"submitted", "unfrozen", "in_review"}:
            raise HTTPException(status_code=400, detail="Заявку нельзя взять в проверку")
        return self.public_request(self.repo.update("requests", request_id, {"status": "in_review", "economist_id": user["id"]}), self.summary(request_id))
    ...
    def fix(self, user: dict, request_id: str) -> dict:
        ...
        items = self._items(request_id)
        if not items:
            raise HTTPException(status_code=400, detail="Нельзя зафиксировать заявку без строк")
        if any(item["status"] == "in_review" for item in items):
            raise HTTPException(status_code=400, detail="Нельзя зафиксировать заявку со строками на рассмотрении")
        self.recalculate_total(request_id)
        return self.public_request(self.repo.update("requests", request_id, {"status": "fixed"}), self.summary(request_id))

    def unfreeze(self, user: dict, request_id: str) -> dict:
        request = get_required(self.repo, "requests", request_id)
        self.permissions.require_economist_review_request(user, request)
        if request["status"] != "fixed":
            raise HTTPException(status_code=400, detail="Разморозить можно только зафиксированную заявку")
        return self.public_request(self.repo.update("requests", request_id, {"status": "unfrozen"}), self.summary(request_id))
