...
Для стандартного запуска дополнительные переменные не требуются: `docker-compose.yml` содержит безопасные значения для локальной разработки. Чтобы переопределить их, создайте `.env` на основе примера:
